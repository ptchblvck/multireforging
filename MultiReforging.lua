...
mysticEnchantReforgeTenTimes:SetPoint("TOP", frame, "TOP", 0, -15)
...
mysticEnchantReforgeTenTimes:SetScript("OnEnter", function(self)
    ...
    GameTooltip:AddLine("Click this button to reforge 10 times", 1, 1, 1)
    GameTooltip:Show()
end)
...
mysticEnchantReforgeAll:SetPoint("TOP", mysticEnchantReforgeTenTimes, "TOP", 0, -25)
...
local function OnClick()
    if mysticEnchantReforgeOnceButton:IsShown() then
        ...
        local mysticRuneCount = GetItemCount(mysticRuneId)
        if clicks < 10 then
            C_Timer.After(1.1, function() mysticEnchantReforgeTenTimes:Click() end)
        else
            clicks = 0
            C_Timer.After(1.1, function() print("|cff00FFFFReforged " .. "|cffA600FF" .. reforgeCount .. "|cff00FFFF times") end)
        end
...
local function OnClickAllRunes()
    if mysticEnchantReforgeOnceButton:IsShown() then
        ...
        if clicks < mysticRuneCount then
            C_Timer.After(1.1, function() mysticEnchantReforgeAll:Click() end)
        else
            clicks = 0
            C_Timer.After(1.1, function() print("|cff00FFFFReforged " .. "|cffA600FF" .. reforgeCount .. "|cff00FFFF times") end)
        end
